...
// 在适当的工具文件中
export function mapApiResponseToProviders(records: any[]): Provider[] {
...
const ProvidersList: FC = () => {
  ...
  console.log('useAllProviders', providers)
  ...
  const onAddProvider = async () => {
    ...
    const { name: prividerName, type } = result
    ...
    const menus = [
      {
        ...
        async onClick() {
          ...
          const result = await AddProviderPopup.show(provider)
          if (result && result.name) {
            updateProvider({ ...provider, name: result.name, type: result.type })
          }
        }
      },
      {
