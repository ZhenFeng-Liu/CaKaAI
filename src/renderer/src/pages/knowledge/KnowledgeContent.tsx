...
const KnowledgeContent: FC<KnowledgeContentProps> = ({ selectedBase }) => {
  ...
  // 添加这段代码来打印选中的知识库全部信息
  console.log('[KnowledgeContent] 选中的知识库完整信息:', {
    selectedBase,
    base,
    items: {
      noteItems,
    ...
    if (files) {
      ...
      try {
        for (const file of uploadedFiles) {
          console.log('[KnowledgeContent] 正在处理文件:', file)

          // 获取添加后的文件项
          const addedFileItem = fileItems.find((item) => (item.content as FileType).path === file.path)

          if (!addedFileItem) {
            console.error('[KnowledgeContent] 未找到添加的文件项:', file.name)
            continue
          }

          console.log('[KnowledgeContent] 找到的文件项:', addedFileItem)

          // 准备发送到后端的数据
          const itemData = {
            uid: base.uid,
            id: addedFileItem.id,
            type: addedFileItem.type,
            content: file,
            created_at: addedFileItem.created_at,
            updated_at: addedFileItem.updated_at,
            uniqueId: addedFileItem.uniqueId,
            uniqueIds: addedFileItem.uniqueIds
          }
          console.log('[KnowledgeContent] 准备发送到后端的数据:', itemData)
          // 调用后端API添加知识库项目
          const response = await knowledgeApi.addKnowledgeItem(itemData)

          console.log(`[KnowledgeContent] 文件 ${file.name} 添加到后端结果:`, response)

          // 处理响应
          if (response.Code !== 0) {
            console.error(`文件 ${file.name} 添加到后端失败:`, response.Msg)
          }
        }
      } catch (error) {
        console.error('向后端添加文件失败:', error)
      }
    }
  }
